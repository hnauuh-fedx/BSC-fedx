...
const EmployeeContent: React.FC<{ data: Extract<DashboardData, { kind: 'EMPLOYEE' }> }> = ({ data }) => <>
  ...
  <Card><CardHeader><CardTitle>Lịch sử BSC gần đây</CardTitle><CardDescription>Các kỳ được cập nhật gần nhất.</CardDescription></CardHeader><CardContent>{data.recentBsc.length === 0 ? <EmptyState/> : <Table><TableHeader><TableRow><TableHead>Kỳ</TableHead><TableHead>Kế hoạch</TableHead><TableHead>Đánh giá</TableHead><TableHead>Điểm</TableHead><TableHead>Xếp loại</TableHead></TableRow></TableHeader><TableBody>{data.recentBsc.map(row => <TableRow key={row.id}><TableCell><Link className="font-medium" to={`/employee-bsc/${row.id}`}>{row.cycleName}</Link></TableCell><TableCell><BscStatusBadge status={row.planStatus}/></TableCell><TableCell><BscStatusBadge status={row.evaluationStatus}/></TableCell><TableCell>{row.officialScore ?? '—'}</TableCell><TableCell>{row.officialGrade ?? '—'}</TableCell></TableRow>)}</TableBody></Table>}</CardContent></Card>
</>;
...
const ManagementContent: React.FC<{ data: ManagementDashboard }> = ({ data }) => <>
  ...
  <Card><CardHeader><CardTitle>Phân bố xếp loại</CardTitle><CardDescription>Chỉ bao gồm BSC đã duyệt đánh giá.</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-5">{Object.entries(data.gradeDistribution).map(([grade, count]) => <Metric key={grade} label={data.grades.find(item => item.value === grade)?.label ?? grade} value={count}/>)}</CardContent></Card>
  <Card><CardHeader><CardTitle>Tiến độ theo phòng ban</CardTitle><CardDescription>Tỷ lệ BSC đã duyệt đánh giá trên tổng số BSC.</CardDescription></CardHeader><CardContent>{data.departmentProgress.length === 0 ? <EmptyState/> : <Table><TableHeader><TableRow><TableHead>Phòng ban</TableHead><TableHead>Đã duyệt</TableHead><TableHead>Tổng BSC</TableHead><TableHead>Tiến độ</TableHead></TableRow></TableHeader><TableBody>{data.departmentProgress.map(row => <TableRow key={row.departmentId}><TableCell className="font-medium">{row.departmentName}</TableCell><TableCell>{row.approvedBsc}</TableCell><TableCell>{row.totalBsc}</TableCell><TableCell>{row.completionPercentage}%</TableCell></TableRow>)}</TableBody></Table>}</CardContent></Card>
