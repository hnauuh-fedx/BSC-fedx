...
vi.mock('../../auth/hooks/use-auth', () => ({
  useAuth: () => ({ user: { permissions: ['bsc.period.manage'] } }),
}));
vi.mock('..', () => ({
  bscCyclesApi: { create: vi.fn(), update: vi.fn(), detail: vi.fn() },
}));
...
describe('BscCycleFormPage', () => {
  ...
  it('creates a cycle without a planned end date or evaluation deadline', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/management/bsc-cycles/new']}>
        <Routes>
          <Route path="/management/bsc-cycles/new" element={<BscCycleFormPage />} />
          <Route path="/management/bsc-cycles/:id" element={<p>Chi tiết kỳ</p>} />
        </Routes>
      </MemoryRouter>,
    );
    ...
    expect(bscCyclesApi.create).toHaveBeenCalledWith({
      code: 'T7', name: 'BSC tháng 7', cycleType: 'MONTH', year: 2026, month: 7, startDate: '2026-07-01',
    });
