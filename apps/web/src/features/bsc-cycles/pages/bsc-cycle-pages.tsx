...
export const BscCycleFormPage: React.FC = () => {
  ...
  return <main className="grid gap-6"><div><h1 className="text-3xl font-semibold">{id ? 'Chỉnh sửa kỳ BSC' : 'Tạo kỳ BSC'}</h1><p className="text-muted-foreground">Kỳ không có deadline; quản trị viên chủ động kết thúc bằng thao tác trên trang chi tiết.</p></div>{error && <ErrorNotice error={error} />}
    <form onSubmit={submit} className="grid gap-6"><Card><CardHeader><CardTitle>Thông tin kỳ tháng</CardTitle><CardDescription>Tháng và ngày bắt đầu không thể đổi sau khi kỳ mở hoặc đã có BSC.</CardDescription></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><Field label="Mã kỳ"><Input value={form.code} onChange={e => set('code', e.target.value.toUpperCase())} required /></Field><Field label="Tên kỳ"><Input value={form.name} onChange={e => set('name', e.target.value)} required /></Field><Field label="Năm"><Input type="number" value={form.year} onChange={e => set('year', e.target.value)} required /></Field><Field label="Tháng"><Input type="number" min="1" max="12" value={form.month} onChange={e => set('month', e.target.value)} required /></Field><Field label="Ngày bắt đầu"><Input type="date" value={form.startDate} onChange={e => set('startDate', e.target.value)} required /></Field></CardContent></Card>
      <div className="flex gap-3"><Button disabled={saving} type="submit">{saving ? 'Đang lưu…' : 'Lưu kỳ'}</Button><Button variant="outline" type="button" onClick={() => navigate(-1)}>Hủy</Button></div></form></main>;
};
...
export const BscCycleDetailPage: React.FC = () => {
  ...
  return <main className="grid gap-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-3xl font-semibold">{cycle.code} — {cycle.name}</h1><Badge className="mt-2" variant="outline">{statusLabel[cycle.status]}</Badge></div><div className="flex flex-wrap gap-2">{canManage(permissions) && ['DRAFT','OPEN'].includes(cycle.status) && <Button variant="outline" onClick={() => navigate(`/management/bsc-cycles/${cycle.id}/edit`)}>Chỉnh sửa</Button>}{canManage(permissions) && actions.map(action => <ConfirmAction key={action} action={action} disabled={acting} requireReason={action === 'close' || (cycle.status === 'LOCKED' && action === 'open')} onConfirm={reason => void transition(action, reason)} />)}</div></div>{error && <ErrorNotice error={error} />}
    <Card><CardHeader><CardTitle>Thông tin kỳ</CardTitle><CardDescription>Kỳ chỉ kết thúc khi quản trị viên xác nhận thao tác.</CardDescription></CardHeader><CardContent className="grid gap-2 text-sm"><p>Loại: Tháng</p><p>Ngày bắt đầu: {formatDate(cycle.startDate)}</p><p>Ngày kết thúc thực tế: {cycle.endDate ? formatDate(cycle.endDate) : 'Chưa kết thúc'}</p><p>Người tạo: {cycle.createdBy?.fullName ?? '—'}</p><p>Cập nhật: {formatTime(cycle.updatedAt)}</p></CardContent></Card>
...
function ConfirmAction({ action, disabled, requireReason = false, onConfirm }: { action: 'open'|'lock'|'close'; disabled: boolean; requireReason?: boolean; onConfirm: (reason?: string) => void }) {
  ...
  const config = { open: ['Mở kỳ BSC', 'Kỳ mở cho phép chủ sở hữu tiếp tục workflow theo trạng thái từng stage.'], lock: ['Khóa kỳ BSC', 'Khóa kỳ sẽ tạm chặn chủ sở hữu tạo, sửa, nộp và duplicate vào kỳ này.'], close: ['Kết thúc kỳ BSC', 'Kết thúc kỳ sẽ ghi nhận ngày kết thúc thực tế và đóng vĩnh viễn kỳ này.'] }[action];
  return <AlertDialog><AlertDialogTrigger asChild><Button disabled={disabled} variant={action === 'close' ? 'destructive' : 'outline'}>{config[0]}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{config[0]}?</AlertDialogTitle><AlertDialogDescription>{config[1]}</AlertDialogDescription></AlertDialogHeader>{requireReason && <Field label={action === 'close' ? 'Lý do kết thúc' : 'Lý do mở lại'}><Textarea value={reason} onChange={event => setReason(event.target.value)} maxLength={500} required /></Field>}<AlertDialogFooter><AlertDialogCancel>Hủy</AlertDialogCancel><AlertDialogAction disabled={requireReason && !reason.trim()} onClick={() => onConfirm(reason.trim() || undefined)}>Xác nhận</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>;
}
