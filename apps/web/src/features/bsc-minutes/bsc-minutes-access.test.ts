...
describe('BSC minutes access', () => {
  it('allows the configured minutes permission and rejects ordinary BSC roles', () => {
    expect(canAccessWorkspacePath('/management/bsc-minutes', ['bsc.minutes.create'])).toBe(true);
    expect(canAccessWorkspacePath('/management/bsc-minutes', ['bsc.minutes.view'])).toBe(false);
