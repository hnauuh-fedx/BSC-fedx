...
export const BscPendingReviewPage: React.FC = () => {
  ...
  const [evaluationDetails, setEvaluationDetails] = useState<Record<string, { bsc: EmployeeBsc; scoring: BscScoringPreview }>>({});
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true); setError(''); setItems([]); setTotal(0); setEvaluationDetails({}); setEvaluationErrors({});
    ...
    try {
      ...
      if (stage === 'EVALUATION') {
        const rows = await Promise.all(result.items.map(async (item) => {
          try {
            const [bsc, scoring] = await Promise.all([employeeBscApi.detail(item.id), employeeBscApi.scoringPreview(item.id)]);
            return { id: item.id, bsc, scoring, error: '' };
          } catch (cause) {
            return { id: item.id, bsc: null, scoring: null, error: cause instanceof Error ? cause.message : 'Không thể tải điểm dự kiến.' };
          }
        }));
        if (generation !== loadGeneration.current) return;
        setEvaluationDetails(Object.fromEntries(rows.filter(row => row.bsc && row.scoring).map(row => [row.id, { bsc: row.bsc!, scoring: row.scoring! }])));
        setEvaluationErrors(Object.fromEntries(rows.filter(row => row.error).map(row => [row.id, row.error])));
      }
    } catch (cause) {
      if (generation !== loadGeneration.current) return;
  ...
  return <PermissionGate anyOf={ALL_REVIEW}>
    <main><PageHeader title="BSC chờ duyệt" action={<Link to="/employee-bsc">Danh sách BSC</Link>}/>
      ...
      {!allowed ? <ErrorState error="Bạn không có quyền xử lý giai đoạn này."/> : <>
        ...
        {loading ? <LoadingState/> : !error && items.length === 0 ? <EmptyState message={stage === 'PLAN' ? 'Không có BSC chờ duyệt nội dung.' : 'Không có BSC chờ duyệt kết quả.'}/> : !error && <table><thead><tr><th>Mã BSC</th><th>Nhân viên</th><th>Kỳ</th><th>Đơn vị</th><th>Ngày nộp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{items.map(item => <Fragment key={item.id}><tr><td><Link to={`/employee-bsc/${item.id}`}>{item.bsc_code}</Link></td><td>{item.users_employee_bsc_employee_idTousers.full_name}</td><td>{item.bsc_cycles.name}</td><td>{item.departments.name}</td><td>{formatDate(stage === 'PLAN' ? item.plan_submitted_at : item.evaluation_submitted_at)}</td><td><BscStatusBadge status={stage === 'PLAN' ? item.plan_status : item.evaluation_status}/></td><td><PermissionGate permission={approvePermission}><button disabled={Boolean(actingId)} onClick={() => void approve(item)}>{actingId === item.id ? 'Đang xử lý…' : 'Duyệt'}</button></PermissionGate> <PermissionGate permission={returnPermission}><button disabled={Boolean(actingId)} onClick={() => { setReturning(item); setReason(''); }}>Trả lại</button></PermissionGate></td></tr>{stage === 'EVALUATION' && <tr><td colSpan={7}>{evaluationErrors[item.id] ? <><ErrorState error={`${evaluationErrors[item.id]} Không hiển thị dữ liệu điểm cũ.`}/><button onClick={() => void load()}>Thử lại</button></> : evaluationDetails[item.id] ? <><BscItemTable bscId={item.id} items={evaluationDetails[item.id].bsc.employee_bsc_items ?? []} scoring={evaluationDetails[item.id].scoring} canManage={false} canUpdateActual={false} onChange={load}/><BscScoreSummary preview={evaluationDetails[item.id].scoring}/></> : <LoadingState/>}</td></tr>}</Fragment>)}</tbody></table>}
