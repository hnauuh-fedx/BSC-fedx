...
export const BscDetailPage: React.FC = () => {
  ...
  return <main>
    ...
    {canDuplicate && <button onClick={() => void openDuplicate()}>Sao chép BSC</button>}
    ...
    {reopenStage && <section role="dialog" aria-modal="true"><h2>{reopenStage === 'PLAN' ? 'Yêu cầu sửa kế hoạch' : 'Yêu cầu sửa kết quả đánh giá'}</h2><p>{reopenStage === 'PLAN' ? 'Khi được duyệt, dữ liệu đánh giá active sẽ được lưu vào lịch sử và đặt lại.' : 'Định nghĩa KPI vẫn khóa; điểm và xếp loại hiện tại sẽ chuyển vào lịch sử.'}</p><label>Lý do<textarea aria-label="Lý do mở lại" maxLength={2000} rows={5} value={reopenReason} onChange={event => setReopenReason(event.target.value)} autoFocus/></label><button disabled={!reopenReason.trim() || mutationPending.current} onClick={() => void requestReopen()}>Gửi yêu cầu</button> <button onClick={() => setReopenStage(null)}>Hủy</button></section>}
    ...
    {duplicateOptions && <section role="dialog" aria-modal="true"><h2>Sao chép BSC</h2><p>Chỉ nội dung kế hoạch BSC đã được duyệt được sao chép. Kết quả thực hiện, TM KQTH, điểm, xếp loại và lịch sử duyệt sẽ không được sao chép.</p><p>Phiên bản nguồn: {duplicateOptions.sourceVersion.versionNumber}; KPI: {duplicateOptions.sourceVersion.summary.itemCount}; tổng trọng số: {String(duplicateOptions.sourceVersion.summary.totalWeight ?? '—')}</p>{duplicateOptions.cycles.length === 0 ? <EmptyState message="Không có kỳ OPEN hợp lệ để sao chép."/> : <><label>Kỳ đích<select aria-label="Kỳ đích" value={targetCycleId} onChange={event => setTargetCycleId(event.target.value)}>{duplicateOptions.cycles.map(cycle => <option key={cycle.id} value={cycle.id}>{cycle.name}</option>)}</select></label><button disabled={!targetCycleId || mutationPending.current} onClick={() => void duplicate()}>Xác nhận sao chép</button></>} <button onClick={() => setDuplicateOptions(null)}>Hủy</button></section>}
    {bsc.plan_status === 'APPROVED' && (scoringLoading ? <LoadingState/> : scoringError ? <><ErrorState error={scoringError}/><button onClick={() => void loadScoring()}>Thử tải lại điểm</button></> : scoring && <><p>{bsc.evaluation_status === 'APPROVED' ? 'Điểm đã duyệt' : 'Điểm dự kiến'}</p><BscScoreSummary preview={scoring}/></>)}
    <BscItemTable bscId={bsc.id} items={items} scoring={scoring} canManage={canManage} canUpdateActual={canActual} onChange={reloadAll}/>
    {permissions.includes(BSC_PERMISSIONS.VIEW_VERSION) && <section><h2>Lịch sử phiên bản</h2>{versionsLoading ? <LoadingState/> : versionsError ? <><ErrorState error={versionsError}/><button onClick={() => void loadVersions()}>Thử lại</button></> : versions.length === 0 ? <EmptyState message="Chưa có phiên bản đã duyệt."/> : <ol>{versions.map(version => <li key={version.id}>Phiên bản {version.versionNumber} — {version.stage} — {version.versionType} — {version.createdBy.full_name}, {formatDate(version.createdAt)} {version.summary.finalGrade ? `— ${String(version.summary.totalScore)} / ${String(version.summary.finalGrade)}` : ''} <button onClick={() => void showVersion(version.id)}>Xem chi tiết</button></li>)}</ol>}{versionDetailLoading && <LoadingState/>}</section>}
    {versionDetail && <section role="dialog" aria-modal="true"><h2>Phiên bản {versionDetail.versionNumber}</h2><p>{versionDetail.stage} — {versionDetail.versionType}</p><pre>{JSON.stringify(versionDetail.snapshot, null, 2)}</pre><button onClick={() => setVersionDetail(null)}>Đóng</button></section>}
    <section><h2>Lịch sử yêu cầu mở lại</h2>{reopenError ? <ErrorState error={reopenError}/> : reopenRequests.length === 0 ? <EmptyState message="Chưa có yêu cầu mở lại."/> : <ol>{reopenRequests.map(request => <li key={request.id}><strong>{request.stage}</strong> — <BscStatusBadge status={request.status}/> — yêu cầu bởi {request.users_bsc_unlock_requests_requested_byTousers.full_name}, {formatDate(request.requested_at)}<br/>Lý do: {request.request_reason}{request.reviewed_at && <><br/>Xử lý bởi {request.users_bsc_unlock_requests_reviewer_idTousers?.full_name ?? '—'}, {formatDate(request.reviewed_at)}{request.review_comment ? `: ${request.review_comment}` : ''}</>}</li>)}</ol>}</section>
