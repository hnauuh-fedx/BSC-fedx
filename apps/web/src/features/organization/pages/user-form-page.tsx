...
const EMPTY_FORM = {
  employeeCode: '', fullName: '', email: '', password: '', departmentId: '', positionId: '', directManagerId: '', roleId: '', roleScopeType: 'SELF' as RoleScopeType,
};
...
export const UserFormPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (edit && userResult.status === 'fulfilled' && userResult.value) {
        ...
        setForm(current => ({ ...current, employeeCode: user.employee_code, fullName: user.full_name, email: user.email, departmentId: user.department_id, positionId: user.position_id, directManagerId: user.direct_manager_id ?? '' }));
      }
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (!form.fullName.trim() || !/^\S+@\S+\.\S+$/.test(form.email) || (!edit && (!form.employeeCode.trim() || form.password.length < 12 || !form.roleId)) || !form.departmentId || !form.positionId) {
  ...
  return <main className="flex flex-col gap-6">
    ...
    {loading ? <LoadingState /> : <form className="flex flex-col gap-6" onSubmit={event => void save(event)} noValidate>
      ...
      {!edit && <section aria-labelledby="role-permission-title" className="rounded-xl border bg-card p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <SelectField label="Vai trò" value={form.roleId} placeholder="Chọn vai trò" onChange={value => setForm({ ...form, roleId: value })} disabled={submitting} helper={roles.length ? `${roles.length} vai trò đang hoạt động.` : 'Không có vai trò khả dụng hoặc tài khoản chưa có quyền xem vai trò.'}>
          ...
          <SelectField label="Phạm vi quyền" value={form.roleScopeType} placeholder="Chọn phạm vi" onChange={value => setForm({ ...form, roleScopeType: value as RoleScopeType })} disabled={submitting}>
