...
export const DepartmentBscListPage: React.FC = () => {
  ...
  return <main className="flex flex-col gap-6">
    <PageHeader title="BSC phòng ban" description="Theo dõi kế hoạch và kết quả của từng phòng ban theo quy trình hai giai đoạn." action={<div className="flex flex-wrap justify-end gap-2">
    ...
    {error && <ErrorState error={error} onRetry={() => void load()}/>} {loading ? <LoadingState/> : items.length === 0 ? <EmptyState message="Chưa có BSC phòng ban trong phạm vi của bạn."/> : <Card>
      ...
      <CardContent><Table className="min-w-[960px]"><TableHeader><TableRow><TableHead>Kỳ</TableHead><TableHead>Phòng ban</TableHead><TableHead>Duyệt nội dung BSC</TableHead><TableHead>Đánh giá kết quả</TableHead><TableHead>Điểm chính thức</TableHead><TableHead>Xếp loại chính thức</TableHead><TableHead className="text-right">Thao tác</TableHead></TableRow></TableHeader>
...
export const DepartmentBscDetailPage: React.FC = () => {
  ...
  return <main aria-label="Chi tiết BSC phòng ban" className="flex flex-col gap-5">
    <PageHeader title={bsc.bsc_code} description={`${bsc.departments.name} · ${bsc.bsc_cycles.name} · Trưởng phòng: ${bsc.responsible_manager.full_name}`} breadcrumb={<Link to="/department-bsc">BSC phòng ban</Link>}>
      <div className="mt-3 flex flex-wrap gap-3 text-sm"><span className="flex items-center gap-2"><span className="text-muted-foreground">Kế hoạch</span><Status value={bsc.plan_status}/></span><span className="flex items-center gap-2"><span className="text-muted-foreground">Kết quả</span><Status value={bsc.evaluation_status}/></span></div>
    </PageHeader>
