...
test('Organization API integration', { skip: integrationEnabled ? false : 'TEST_DATABASE_URL is not configured with a safe test database' }, async t => {
  ...
  const employeeRole = await prisma.roles.findUniqueOrThrow({ where: { code: 'EMPLOYEE' } });
  ...
  const userCreate = await prisma.permissions.findUniqueOrThrow({ where: { code: 'user.create' } });
  const permissionAssign = await prisma.permissions.findUniqueOrThrow({ where: { code: 'permission.assign' } });
  ...
  await prisma.role_permissions.createMany({ data: [{ role_id: deptRole.id, permission_id: userView.id }, { role_id: deptRole.id, permission_id: userCreate.id }, { role_id: deptRole.id, permission_id: permissionAssign.id }, { role_id: selfRole.id, permission_id: userView.id }] });
  ...
  const adminToken = await login(admin.username), noPermissionToken = await login(noPermission.username), departmentToken = await login(departmentUser.username), selfToken = await login(selfUser.username);
  ...
  try {
    ...
    await t.test('users enforce scopes, secure create, manager history, revocation and last-admin protection', async () => {
      ...
      await request(server).get('/users/filter-options').expect(401); await request(server).get('/users/filter-options').set(auth(noPermissionToken)).expect(403);
      const departmentOptions = await request(server).get('/users/filter-options').set(auth(departmentToken)).expect(200);
      assert.deepEqual(departmentOptions.body.departments.map((item: { id: string }) => item.id), [departmentA.id]);
      assert.ok(departmentOptions.body.positions.some((item: { id: string }) => item.id === positionA.id));
      ...
      assert.equal(invalidDirector.status, 400); assert.equal(invalidDirector.body.code, 'USER_DIRECTOR_SCOPE_GLOBAL_REQUIRED');
      const globalDirector = await request(server).post('/users').set(auth(adminToken)).send({ ...createBody, employeeCode: `${marker}_DIRECTOR_GLOBAL`, username: `${marker}_director_global`.toLowerCase(), fullName: 'Director Global Scope', email: `${marker.toLowerCase()}_director_global@example.test`, directManagerId: null, roleId: directorRole.id, roleScopeType: 'GLOBAL' }).expect(201);
      ids.users.push(globalDirector.body.id);
      const directorAssignment = await prisma.user_roles.findFirstOrThrow({ where: { user_id: globalDirector.body.id, role_id: directorRole.id } });
      assert.equal(directorAssignment.scope_type, 'GLOBAL'); assert.equal(directorAssignment.scope_id, null);
      await request(server).post('/users').set(auth(adminToken)).send({ ...createBody, employeeCode: `${marker}_NO_ROLE`, email: `${marker.toLowerCase()}_no_role@example.test`, roleId: undefined }).expect(400);
      await request(server).post('/users').set(auth(departmentToken)).send({ ...createBody, employeeCode: `${marker}_ESCALATED_ROLE`, email: `${marker.toLowerCase()}_escalated_role@example.test`, directManagerId: departmentUser.id }).expect(403);
      await request(server).post('/users').set(auth(departmentToken)).send({ ...createBody, employeeCode: `${marker}_GLOBAL_SCOPE`, email: `${marker.toLowerCase()}_global_scope@example.test`, directManagerId: departmentUser.id, roleId: deptRole.id, roleScopeType: 'GLOBAL' }).expect(403);
      await request(server).post('/users').set(auth(departmentToken)).send({ ...createBody, employeeCode: `${marker}_OUTSIDE_DEPT`, email: `${marker.toLowerCase()}_outside_dept@example.test`, departmentId: departmentB.id, directManagerId: otherUser.id, roleId: deptRole.id, roleScopeType: 'DEPARTMENT' }).expect(403);
      await request(server).post('/users').set(auth(departmentToken)).send({ ...createBody, employeeCode: `${marker}_OUTSIDE_MANAGER`, email: `${marker.toLowerCase()}_outside_manager@example.test`, directManagerId: otherUser.id, roleId: deptRole.id, roleScopeType: 'DEPARTMENT' }).expect(403);
      const managerWithExternalAncestor = await createUser('MGR_EXT', departmentA.id, positionA.id, 'ACTIVE', otherUser.id);
      const scopedCreated = await request(server).post('/users').set(auth(departmentToken)).send({ ...createBody, employeeCode: `${marker}_SCOPED_CREATE`, username: `${marker}_scoped_create`.toLowerCase(), email: `${marker.toLowerCase()}_scoped_create@example.test`, directManagerId: managerWithExternalAncestor.id, roleId: deptRole.id, roleScopeType: 'DEPARTMENT' }).expect(201); ids.users.push(scopedCreated.body.id);
      const managerOptions = await request(server).get('/users/filter-options').set(auth(departmentToken)).expect(200);
      assert.ok(managerOptions.body.directManagers.some((item: { id: string }) => item.id === managerWithExternalAncestor.id));
      ...
      const historyBefore = await prisma.manager_relationships.count({ where: { employee_id: created.body.id } }); await request(server).patch(`/users/${created.body.id}`).set(auth(adminToken)).send({ directManagerId: managerA.id, transferReason: 'Phân công quản lý trực tiếp' }).expect(200); assert.equal(await prisma.manager_relationships.count({ where: { employee_id: created.body.id } }), historyBefore + 1);
      ...
      await request(server).patch(`/users/${created.body.id}`).set(auth(adminToken)).send({ directManagerId: null, transferReason: 'Gỡ quản lý trực tiếp' }).expect(200); assert.equal(await prisma.manager_relationships.count({ where: { employee_id: created.body.id, end_date: null, is_primary: true } }), 0);
