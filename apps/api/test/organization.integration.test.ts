...
test('Organization API integration', { skip: integrationEnabled ? false : 'TEST_DATABASE_URL is not configured with a safe test database' }, async t => {
  ...
  try {
    ...
    await t.test('users enforce scopes, secure create, manager history, revocation and last-admin protection', async () => {
      ...
      const createBody = { employeeCode: `${marker}_API_USER`, fullName: 'API User', email: `${marker.toLowerCase()}_api@example.test`, password, departmentId: departmentA.id, positionId: positionA.id, directManagerId: departmentUser.id, roleId: employeeRole.id, roleScopeType: 'SELF' };
