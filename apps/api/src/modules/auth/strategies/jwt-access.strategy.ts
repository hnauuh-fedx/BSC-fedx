...
export class JwtAccessStrategy extends PassportStrategy(Strategy, 'jwt-access') {
  ...
  async validate(payload: AccessTokenPayload): Promise<AuthUser> {
    ...
    const roles = user.user_roles_user_roles_user_idTousers.map((assignment) => ({ code: assignment.roles.code, scopeType: assignment.scope_type as AuthUser['roles'][number]['scopeType'], scopeId: assignment.scope_id }));
