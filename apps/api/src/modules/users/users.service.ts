...
export class UsersService {
  ...
  private async validateRoleAssignment(db: Db, actor: AuthUser, roleId: string, scopeType: 'GLOBAL' | 'DEPARTMENT' | 'SELF', departmentId: string) {
    ...
    if (role.code === 'DIRECTOR' && scopeType !== 'GLOBAL') {
      throw new BadRequestException({
        code: 'USER_DIRECTOR_SCOPE_GLOBAL_REQUIRED',
        message: 'Vai trò Giám đốc phải sử dụng phạm vi toàn hệ thống.',
      });
    }
  ...
  async findAll(actor: AuthUser, q: UserQueryDto) { const scopeWhere = this.whereForScope(actor); if (q.departmentId && !this.scope.canAccessDepartment(actor, q.departmentId)) throw new ForbiddenException({ code: 'AUTH_SCOPE_DENIED', message: 'Bạn không có quyền truy cập đơn vị này.' }); const filters: Prisma.usersWhereInput[] = [scopeWhere, { deleted_at: null }]; if (q.departmentId) filters.push({ department_id: q.departmentId }); if (q.positionId) filters.push({ position_id: q.positionId }); if (q.directManagerId) filters.push({ direct_manager_id: q.directManagerId }); if (q.status) filters.push({ status: q.status }); if (q.search) filters.push({ OR: [{ username: { contains: q.search, mode: 'insensitive' } }, { employee_code: { contains: q.search, mode: 'insensitive' } }, { full_name: { contains: q.search, mode: 'insensitive' } }, { email: { contains: q.search, mode: 'insensitive' } }] }); const where: Prisma.usersWhereInput = { AND: filters }; const [items, total] = await this.prisma.$transaction([this.prisma.users.findMany({ where, select: safeUser, orderBy: { [q.sortBy]: q.sortOrder }, skip: (q.page - 1) * q.limit, take: q.limit }), this.prisma.users.count({ where })]); return { items, page: q.page, limit: q.limit, total }; }
  async filterOptions(actor: AuthUser) {
    const where: Prisma.usersWhereInput = { AND: [this.whereForScope(actor), { deleted_at: null }] };
    const [departmentGroups, positionGroups, managerGroups] = await Promise.all([
      this.prisma.users.groupBy({ by: ['department_id'], where }),
      this.prisma.users.groupBy({ by: ['position_id'], where }),
      this.prisma.users.groupBy({ by: ['direct_manager_id'], where: { AND: [where, { direct_manager_id: { not: null } }] } }),
    ]);
    const [departments, positions, directManagers] = await Promise.all([
      this.prisma.departments.findMany({ where: { id: { in: departmentGroups.map(item => item.department_id) } }, select: { id: true, code: true, name: true, status: true } }),
      this.prisma.positions.findMany({ where: { id: { in: positionGroups.map(item => item.position_id) } }, select: { id: true, code: true, name: true, level: true, status: true } }),
      this.prisma.users.findMany({ where: { id: { in: managerGroups.flatMap(item => item.direct_manager_id ? [item.direct_manager_id] : []) } }, select: { id: true, employee_code: true, full_name: true, status: true } }),
    ]);
    return {
      departments: departments.sort((a, b) => a.name.localeCompare(b.name, 'vi')),
      positions: positions.sort((a, b) => a.name.localeCompare(b.name, 'vi')),
      directManagers: directManagers.sort((a, b) => a.full_name.localeCompare(b.full_name, 'vi')),
    };
  }
